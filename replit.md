...
- **Plan System**:
  ...
  - **Scale Plan** (900 PKR, 10 days): VEO + Bulk generation (max 50 prompts, batch=7, delay=30s, 1000 videos/day limit)
  - **Empire Plan** (1500 PKR, 10 days): All tools - VEO + Bulk (max 100 prompts, batch=20, delay=15s, 2000 videos/day) + Script Generator + Text-to-Image + Image-to-Video
...
- **Bulk Generation**: Backend queue system with configurable batch processing. Uses round-robin token rotation for videos. Processing continues in the background even if the user leaves the page. Prompt limits, batch size, and delay are enforced based on user plan (Scale: 50 prompts max, batch=7, delay=30s; Empire: 100 prompts max, batch=20, delay=15s).
...
- **Status Polling**: All video generation status checks (VEO, Bulk, Image-to-Video, Regeneration) use 15-second polling intervals to reduce server load and improve scalability for 100+ users. Background polling and internal VEO status checks also use 15-second intervals for consistency.
